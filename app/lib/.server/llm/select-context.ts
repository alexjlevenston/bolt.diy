...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyCodeAgentActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyCodeAgentActions(content);

      content = content.replace(/<div class=\\"__codeagentThought__\\">.*?<\/div>/s, '');
