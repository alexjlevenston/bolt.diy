...
export function simplifyCodeAgentActions(input: string): string {
  // Using regex to match codeagentAction tags that have type="file"
  const regex = /(<codeagentAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/codeagentAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<codeagentAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</codeagentAction>`;
    });

  return `<codeagentArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</codeagentArtifact>`;
}
